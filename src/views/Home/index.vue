  ...
  <div class="home">
    ...
    <div v-for="item in homeList" :key="item.id">
      <div class="activity-panel" v-if="item.type === 1">
        <!-- 活动版块内容 -->
        <el-row>
          <el-col class="content" :span="8" v-for="o in item.panelContents" :key="'info'+o.id">
            <!-- :offset="index > 0 ? 2 : 0" -->
      ...
      <section class="w mt30 clearfix" v-if="item.type === 2">
        <s-m-shelf :title="item.name">
          <div slot="content" class="hot">
            <s-m-goods v-for="o in item.panelContents" :key="o.id" :goods="o"></s-m-goods>
          </div>
        </s-m-shelf>
      </section>
...
<script>
import SMShelf from "@/components/Shelf";
